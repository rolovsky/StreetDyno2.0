...
<head>
    ...
    <style>
        ...
        .status-bar { padding:15px; background:#222; border-radius:10px; margin-bottom:15px; text-align:center; border:1px solid #444; font-weight:bold; }
        .card { background:#1a1a1a; padding:20px; border-radius:15px; border:1px solid #333; text-align:center; margin-bottom:12px; }
        .label { color:#888; font-size:1.1em; text-transform:uppercase; margin-bottom:5px; }
        .value { font-size:4.8em; font-weight:bold; font-family: monospace; line-height:1em; }
        .btn { display:block; padding:22px; border-radius:12px; text-decoration:none; font-weight:bold; text-align:center; margin-top:15px; font-size:1.4em; background:#00ffcc; color:#111; }
    </style>
</head>
<body>
    <div id="status" class="status-bar">Warte auf Arduino...</div>
    ...
    <a href="/logs" class="btn">📂 LOGS & ANALYSE</a>
    <script>
        setInterval(() => {
...
def analyze_file():
    ...
    try:
        ...
        df['rpm_s'] = df['RPM'].rolling(window=21, center=True).median()
        ...
        return f'<body style="background:#111; color:white; text-align:center; padding:20px;">' \
               f'<h1>{df["hp_s"].max():.1f} PS</h1><img src="/plots/{pname}" style="width:100%; border-radius:10px;">' \
               f'<br><br><a href="/logs" style="color:#ff9800; font-size:1.5em;">ZURÜCK</a></body>'
...
def hardware_loop():
    ...
    while True:
            ...
            if ser.in_waiting > 0:
                ...
                if line.startswith('$'):
                    ...
                    if len(parts) >= 3:
                        ...
                        if last_valid_rpm > 500 and abs(raw_rpm - last_valid_rpm) > 3000:
                            proc_rpm = last_valid_rpm
                        else:
                            proc_rpm = raw_rpm
                            last_valid_rpm = raw_rpm

                        rpm_window.pop(0); rpm_window.append(proc_rpm)
        ...
        if time.time() - last_upd > 0.1:
            oled.show_status(telemetry["rpm"], telemetry["speed"], telemetry["afr"], telemetry["egt"], "V3.9", telemetry["fix"], logger.is_logging)
            last_upd = time.time()
        time.sleep(0.005)
