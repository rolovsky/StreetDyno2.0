...
# ==========================================
# --- KONFIGURATION v4.9 (SMART SYNC + SMOOTH AFR) ---
# ==========================================
...
<body>
    <div id="status" class="status-bar">V4.9 READY</div>
...
def hardware_loop():
    ...
    while True:
            ...
            if ser.in_waiting > 0:
                ...
                if line.startswith('$'):
                    ...
                    if len(parts) >= 3:
                        ...
                        # LOGGING (Trigger auf Rohdaten!)
                        if not logger.is_logging:
                            if target_rpm > AUTO_START_RPM and spd > MIN_SPEED_KMH:
                                logger.filename = os.path.join(LOG_DIR, f"dyno_log_{datetime.now().strftime('%Y%m%d-%H%M%S')}.csv")
                                logger.start(); telemetry["status"]="🔴 REC"
                        else:
                            # Logge das gedämpfte AFR
                            logger.log(target_rpm, current_filtered_afr, p_egt, spd, g.lat, g.lon, g.fix if g else False)
                            if target_rpm < 1100 and spd < 1.0:
        ...
        if time.time() - last_upd > 0.1:
            oled.show_status(telemetry["rpm"], telemetry["speed"], telemetry["afr"], telemetry["egt"], "V4.9", telemetry["fix"], logger.is_logging)
            last_upd = time.time()
        time.sleep(0.005)
